...
func (dq *DepositQuery) QueryUser() *UserQuery {
...
func (dq *DepositQuery) WithUser(opts ...func(*UserQuery)) *DepositQuery {
	query := (&UserClient{config: dq.config}).Query()
...
func (dq *DepositQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Deposit, error) {
	...
	if query := dq.withUser; query != nil {
		if err := dq.loadUser(ctx, query, nodes, nil,
			func(n *Deposit, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepositQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Deposit, init func(*Deposit), assign func(*Deposit, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Deposit)
	for i := range nodes {
		if nodes[i].user_deposits == nil {
			continue
		}
		fk := *nodes[i].user_deposits
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_deposits" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
