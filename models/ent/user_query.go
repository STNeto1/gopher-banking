...
func (uq *UserQuery) QueryDeposits() *DepositQuery {
	query := (&DepositClient{config: uq.config}).Query()
...
func (uq *UserQuery) WithDeposits(opts ...func(*DepositQuery)) *UserQuery {
	query := (&DepositClient{config: uq.config}).Query()
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withDeposits; query != nil {
...
func (uq *UserQuery) loadDeposits(ctx context.Context, query *DepositQuery, nodes []*User, init func(*User), assign func(*User, *Deposit)) error {
	...
	query.Where(predicate.Deposit(func(s *sql.Selector) {
		s.Where(sql.InValues(user.DepositsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_deposits
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_deposits" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
